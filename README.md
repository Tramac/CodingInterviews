...
1.选择在模式上向后移动两个字符,这相当于'*'和它前面的字符被忽略掉了,因为此时'*'代表的是匹配字符串中的0个字符<br>
...
3.判断条件从模式的下一个字符是否为'*'开始.即```if(*(pattern + 1) == '*')```<br>
4.如果模式的下一个字符为'*',那么判断当前的字符是否相同(包括模式为'.')<br>
5.如果相同,可以选择字符串向右移动一位而模式保持不动,此时代表"*"复制了前一个字符.或者选择字符串向右移动一位,模式向右移动两位,此时代表'*'匹配后舍弃了前一个字符.或者字符串不变,模式向右移动两位,直接舍弃前一个字符<br>
